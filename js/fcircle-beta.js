  ...
  var loadMoreBtn = `
    ...
    <div id="cf-footer" class="cf-new-add">
     ...
     <span class="cf-data-lastupdated">更新于：${sdata.last_updated_time}</span>
      Powered by <a target="_blank" href="https://github.com/Rock-Candy-Tea/hexo-circle-of-friends">FriendCircle</a>
    </div>
...
function loadArticleItem(datalist,start,end){
  ...
  if(start &lt; articleNum){
    for (var i = start;i<endfor;i++){ var="" item="datalist[i];" articleitem="" +="`" <div="" class="cf-article">
        ...
        <div class="cf-article-avatar no-lightbox flink-item-icon">
          <img class="cf-img-avatar avatar lazyload" src="${item.avatar}" data-srcset="${item.avatar}" srcset="data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" alt="avatar" onerror="this.src='${fdata.error_img}'; this.onerror = null;">
  ...
  var showHtml = `
      <div class="cf-overshow">
        <div class="cf-overshow-head">
          <img class="cf-img-avatar avatar lazyload" src="${userinfo.avatar}" data-srcset="${userinfo.avatar}" srcset="data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" alt="avatar" onerror="this.src='${fdata.error_img}'; this.onerror = null;">
          <a class="" target="_blank" rel="noopener nofollow" href="${userinfo.link}">${userinfo.author}</a>
        </div>
        <div class="cf-overshow-content">
  `
  for (var i = 0;i<userinfo.article_num;i++){ var="" item="articledata[i];" showhtml="" +="`" <p=""><a class="cf-article-title" href="${item.link}" target="_blank" rel="noopener nofollow" data-title="${item.title}">${item.title}</a><span>${item.created}</span><p></p>
    `
  }
  showHtml += '</userinfo.article_num;i++){></div></div>'
...
function loadNextArticle(){
  ...
  var articleItem = ""
    for (var i = 0;i<nextarticle.length;i++){ var="" item="nextArticle[i];" articleitem="" +="`" <div="" class="cf-article">
        <a class="cf-article-title" href="${item.link}" target="_blank" rel="noopener nofollow" data-title="${item.title}">${item.title}</a>
        <span class="cf-article-floor">${item.floor}</span>
        <div class="cf-article-avatar no-lightbox flink-item-icon">
          <img class="cf-img-avatar avatar lazyload" src="${item.avatar}" data-srcset="${item.avatar}" srcset="data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" alt="avatar" onerror="this.src='${fdata.error_img}'; this.onerror = null;">
